...
#ifdef BH_DEBUG
#define BH_DBG(fmt, args...) printk(KERN_INFO "%s: " fmt, DRV_NAME, ##args )
#else
...
{
	struct bh_event *event = container_of(work, struct bh_event, work.work);
	int ret = 0;
	...
	BH_DBG("after HZ, gpio value %lu-'%d'\n", jiffies, gpio_button_get_value(event->bdata));
...
static int button_hotplug_create_event(struct gpio_keys_button_data *bdata, const char *name, unsigned int type,
		unsigned long seen, int pressed)
{
	struct bh_event *event;
	unsigned long delay = msecs_to_jiffies(1000); //msecs_to_jiffies(pdata->poll_interval);
	...
	INIT_DELAYED_WORK(&event->work, button_hotplug_work);

	if (delay >= HZ)
		delay = round_jiffies_relative(delay);
	schedule_delayed_work(&event->work, delay);
			   ...
			   unsigned int type, int value)
{
	...
	button_hotplug_create_event(data,button_map[btn].name, type,
...
static int gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (ret < 0)
			...
			dev_info(&pdev->dev, "gpio:%d has irq:%d\n", button->gpio, button->irq);
